...
def run_capacity_analysis():
    ...
    while True:
        try:
            adet_input = console.input("\n[bold cyan]Soru:[/bold cyan] Bu reçeteden TOPLAM KAÇ ADET (set) üretmek istiyorsunuz? ").strip()
    ...
    haric_tut = Confirm.ask("[bold yellow]Soru:[/bold yellow] Merdane, boru, kaynak vb. parçalar (haric_tutulacak_parcalar.json listesindeki) yoksayılsın mı?", default=False)
    ...
    if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
        ...
        if os.path.isfile(dragged_path) and dragged_path.lower().endswith(('.xlsx', '.xls')):
            ...
            input_dir = dragged_path if os.path.isdir(dragged_path) else os.path.dirname(dragged_path)
            ...
            if not excel_files:
                console.print("[red]Sürüklediğiniz klasörde analiz edilecek Reçete Excel'i bulunamadı![/red]")
                return
    ...
    for c in df.columns:
        nc = norm_col(str(c))
        if nc in ["kod", "malzeme kodu"]: kod_col = c
        if nc in ["sıra no", "sira no"]: sira_col = c
    ...
    for target in ["Çarpılmış Miktar", "Carpilmis Miktar", "Miktar"]:
        for c in df.columns:
            if target.lower() in str(c).lower():
                miktar_col_fiziki = c
                break
        if miktar_col_fiziki: break
        
    if not miktar_col_kull: miktar_col_kull = miktar_col_fiziki
    if not miktar_col_fiziki: miktar_col_fiziki = miktar_col_kull
            
    if not kod_col or not miktar_col_fiziki:
        console.print("[red]Dosyada Kod veya Çarpılmış Miktar/Rezerve Edilecek Miktar sütunu bulunamadı![/red]")
        return
        
    op_cols = []
    for c in df.columns:
        if "operasyon" in norm_col(str(c)) and "adi" not in norm_col(str(c)) and "sira" not in norm_col(str(c)):
            op_cols.append(c)
    ...
    def scale_df(df_to_scale):
        ...
        target_quantities_keywords = [
            "miktar", "çarpılmış miktar", "carpilmis miktar", 
            "rezerve edilecek miktar", "aktif rezerve edilen miktar", 
            "kalan", "cikan"
        ]
    ...
    for stock_basis in stock_bases:
        ...
        try:
            ...
            do_match_depo(temp_input_path, group=selected_group_name, file_totals=file_totals, raw_df=df_raw_scaled, input_path=file_path, stock_basis=stock_basis)
        except Exception as e:
        ...
        for c in df.columns:
            ...
            elif nc == "stok" or (nc == "miktar" and "kritik" not in nc and "rezerve" not in nc and c != miktar_col_fiziki and c != miktar_col_kull):
                if nc == "stok":
        ...
        if not col_kull: col_kull = col_fiziki
        if not col_fiziki: col_fiziki = col_kull
        ...
        for kod, birim_ihtiyac in source_needs_kull.items():
            if birim_ihtiyac <= 0: continue
            stok_kull = stok_dict_kullanilabilir.get(kod, 0.0)
            max_k_this = math.floor(stok_kull / birim_ihtiyac)
            if max_k_this < max_k_kullanilabilir: max_k_kullanilabilir = max_k_this
        ...
        for kod, birim_ihtiyac in source_needs_fiz.items():
            ...
            if max_k_this < max_k_fiziki: max_k_fiziki = max_k_this
            toplam_ihtiyac = birim_ihtiyac * hedef_adet
        ...
        if max_k_kullanilabilir == 999999: max_k_kullanilabilir = 0
        if max_k_fiziki == 999999: max_k_fiziki = 0
                
        # Set bazlı yüzdelik hesaplama
        set_yuzdeleri = []
        for k in range(1, hedef_adet + 1):
            if stock_basis == "kullanilabilir":
                eksik_kull_say = sum(1 for kod, birim in source_needs_kull.items() if stok_dict_kullanilabilir.get(kod, 0.0) < (birim * k))
                toplam_kull = len(source_needs_kull)
                y_kull = ((toplam_kull - eksik_kull_say) / toplam_kull) * 100 if toplam_kull > 0 else 0
                ...
                eksik_fiz_say = sum(1 for kod, birim in source_needs_fiz.items() if stok_dict_fiziki.get(kod, 0.0) < (birim * k))
        ...
        if stock_basis == "kullanilabilir":
            console.print(f"[bold yellow]=> Kullanılabilir Stoğa Göre Kapasite:[/bold yellow] [bold white]{max_k_kullanilabilir} Adet[/bold white]")
        else:
            console.print(f"[bold cyan]=> Fiziki Stoğa Göre Kapasite:[/bold cyan] [bold white]{max_k_fiziki} Adet[/bold white]")
        ...
        rule_blue = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=100, color="638EC6", showValue="None", minLength=None, maxLength=None)
        rule_green = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=100, color="5CB85C", showValue="None", minLength=None, maxLength=None)
        ...
        for sh_name in active_sheets:
            ...
            if sh_name == "Kapasite Simülasyonu":
                ...
                if col_kull_oran:
                    ws.conditional_formatting.add(f"{col_kull_oran}2:{col_kull_oran}{ws.max_row}", rule_blue)
                if col_fiz_oran:
                    ws.conditional_formatting.add(f"{col_fiz_oran}2:{col_fiz_oran}{ws.max_row}", rule_green)
