...
from recipe_automation.services.matcher import match_with_depo, append_hammadde, append_carpimis_miktar
from recipe_automation.core.config import settings
import warnings

def norm_col(text: str) -> str:
    if not text: return ""
    replacements = {'ı':'i', 'İ':'i', 'I':'i', 'ş':'s', 'Ş':'s', 'ğ':'g', 'Ğ':'g', 'ü':'u', 'Ü':'u', 'ö':'o', 'Ö':'o', 'ç':'c', 'Ç':'c'}
    t = str(text)
...
def load_group(group_name: str) -> set[str]:
    ...
    if not os.path.exists(json_path):
        console.print(f"[red]Hata:[/red] {json_path} dosyası bulunamadı. Lütfen klasörde oluşturun.")
        raise typer.Exit(1)
    ...
    if group_name not in data:
        console.print(f"[red]Hata:[/red] '{group_name}' grubu JSON dosyasında bulunamadı. Mevcut gruplar: {list(data.keys())}")
        raise typer.Exit(1)
...
def print_report(file_name: str, meta: dict[str, str | int | list[str]]) -> None:
    ...
    panel = Panel(table, title=f"[bold green]Rapor: {file_name}[/bold green]", border_style="green", expand=False)
    console.print(panel)
...
def aggregate_toplu_liste(df: pd.DataFrame) -> pd.DataFrame:
    ...
    carp_col = None
    exact_targets = [
        "Çarpılmış Miktar", "Carpilmis Miktar",
    ...
    df[carp_col] = pd.to_numeric(df[carp_col].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
...
def scan(path: str = typer.Argument(..., help="Excel dosya veya klasör yolu")) -> None:
    ...
    for file in files:
        ...
        console.print(f"[cyan]{os.path.basename(file)}[/cyan] -> Bulunan operasyon sütunları: {cols}")
        if not cols:
             console.print(f"[yellow]Uyarı:[/yellow] Bu dosyada 'Operasyon' kelimesi içeren sütun bulunamadı.")
    ...
    if current_operations:
        ...
        if new_operations:
            items_str = "\n".join([f"[bold white] - {op}[/bold white]" for op in sorted(list(new_operations))])
            panel = Panel(
                items_str,
            ...
            for new_op in sorted(list(new_operations)):
                ...
                choice_str = Prompt.ask("Seçiminiz", choices=[str(x) for x in range(1, len(group_keys)+3)])
                ...
                if choice <= len(group_keys):
                    ...
                    console.print(f"[green]✅ '{new_op}', '{selected_group}' grubuna eklendi.[/green]")
                    ...
                    console.print(f"[green]✅ '{new_group_name}' oluşturuldu ve operasyon eklendi.[/green]")
                else:
                    console.print(f"[yellow]⏭️ Atlandı. Sadece geçmişe kaydedildi.[/yellow]")
        ...
        console.print(f"[green]✅ Başarılı![/green] Geçmiş dosyasında toplam {len(all_operations)} benzersiz operasyon kayıtlı.")
        console.print(f"Geçmiş dosyası: {history_path}")
    else:
        console.print("[yellow]Uyarı:[/yellow] Hiç operasyon bulunamadı.")
@app.command()
def auto_filter(path: str = typer.Argument(..., help="Excel dosya veya klasör yolu"),
                group: str = typer.Option("TIM", help="Filtrelenecek operasyon grubu (JSON'dan okur)")) -> None:
    ...
    if df is not None:
        ...
        if has_id and has_rezerve:
            console.print("[green]✅ 'Sıra No' ve 'Rezerve Edilecek Miktar' sütunları bulundu. -> [bold]ID (Kırılım) Filtresi[/bold] başlatılıyor...[/green]")
            filter_id(path, group)
        else:
            console.print("[green]✅ Klasik kırılım sütunları bulunamadı. -> [bold]Stok (Envanter) Filtresi[/bold] başlatılıyor...[/green]")
...
@app.command()
def filter_id(path: str = typer.Argument(..., help="Excel dosya veya klasör yolu"),
              group: str = typer.Option("TIM", help="Filtrelenecek operasyon grubu (JSON'dan okur)")) -> None:
    """ID (Sıra No) ve Rezerve Edilecek Miktar bazlı filtreleme yapar."""
    db_dir = os.path.join(os.getcwd(), settings.db_dir_name)
    priority_file = os.path.join(db_dir, "oncelik_sirasi.json")
    if not Confirm.ask(f"[bold yellow]DİKKAT:[/bold yellow] Kaynak dosyalar için [white]'oncelik_sirasi.json'[/white] dosyasındaki öncelik sıralamasını güncellediniz mi?"):
    ...
    if os.path.exists(json_path):
        ...
        if groups_data:
            ...
            for i, g in enumerate(group_keys, 1):
                console.print(f"  [{i}] {g}")
            choice_str = Prompt.ask("Seçiminiz", choices=[str(x) for x in range(1, len(group_keys)+1)])
            group = group_keys[int(choice_str)-1]

    console.print("\n[bold cyan]=== FİLTRELEME İŞLEMİ BAŞLATILIYOR ===[/bold cyan]")
    haric_tut = False
    if Confirm.ask("[bold yellow]Soru:[/bold yellow] Merdane, boru, kaynak vb. parçalar (haric_tutulacak_parcalar.json listesindeki) yoksayılsın mı?", default=False):
        haric_tut = True
        
    target_ops = load_group(group)
    console.print(f"[bold blue]Kullanılan Grup:[/bold blue] {group} ({len(target_ops)} adet operasyon kuralı)")
    
    files = find_excel_files(path)
    if not files:
        console.print(f"[red]Hata:[/red] Dosya bulunamadı.")
        raise typer.Exit(1)
        ...
        for h in (0, 2):
            ...
            try:
                ...
                if not res.empty:
                    ...
                    if carp_col:
                        console.print(f"  [cyan]ℹ️ Miktar sütunu olarak '{carp_col}' tespit edildi ve kullanıldı.[/cyan]")
                        res.insert(0, "KAYNAK DOSYA", kaynak_metin)
                    else:
                        console.print(f"  [yellow]⚠️ Miktar sütunu BULUNAMADI! Varsayılan birleştirme uygulanıyor.[/yellow]")
                        res.insert(0, "KAYNAK DOSYA", kaynak_metin)
                    all_dfs.append(res)
                    df_raw_copy = df.copy()
                    ...
                    if len(files) == 1:
                        out_path = os.path.join(os.path.dirname(path), f"{group}_{os.path.basename(path)}")
                        ...
                        do_match_depo(out_path, group=group, file_totals=file_totals, raw_df=df, input_path=path)
                success = True
                break
    ...
    if len(files) > 1 and all_dfs:
        ...
        do_match_depo(out_path, group=group, file_totals=file_totals, raw_df=combined_raw, input_path=path)

    if genel_silinen_kodlar:
        console.print("\n[bold red]=== JSON HARİÇ TUTMA ÖZETİ ===[/bold red]")
        console.print(f"[yellow]Tüm dosyalarda toplam {len(genel_silinen_kodlar)} adet benzersiz parça (ve alt kırılımları) silindi.[/yellow]")
        console.print(f"[bold red]Silinen Tüm Kodlar:[/bold red] {', '.join(sorted(list(genel_silinen_kodlar)))}\n")

@app.command()
def filter_stock(path: str = typer.Argument(..., help="Excel dosya veya klasör yolu"),
                 group: str = typer.Option("TIM", help="Filtrelenecek operasyon grubu (JSON'dan okur)")) -> None:
...
@app.command()
def match_depo(path: str = typer.Argument(..., help="Filtrelenmiş Excel dosyasının yolu"),
               group: str = typer.Option("TIM", help="Hangi grup için eşleştirme yapıldığı")) -> None:
...
def do_match_depo(path: str, group: str = "TIM", file_totals: dict = None, raw_df: pd.DataFrame = None, input_path: str = None, stock_basis: str = "kullanilabilir") -> None:
    ...
    if not os.path.exists(db_dir):
        os.makedirs(db_dir, exist_ok=True)
        console.print(f"[yellow]Uyarı:[/yellow] '{settings.db_dir_name}' klasörü bulunamadığı için otomatik oluşturuldu.")
        console.print("[red]Hata:[/red] Lütfen 'TumRotaBilgileri.xlsx' ve 'ReceteTumRotaListe.xlsx' dosyalarını bu klasöre koyup tekrar deneyin.")
        raise typer.Exit(1)
    ...
    console.print(f"[bold blue]2. Veritabanı:[/bold blue] ReceteTumRotaListe.xlsx")
    ...
    try:
        ...
        if found_cols:
            ...
            for col in found_cols:
                ...
                ans = Confirm.ask(f"[white]'{col}'[/white] sütununda değeri '0' veya 'Hayır' (Pasif) olan satırları LİSTEDEN SİLELİM Mİ?", default=True)
                if ans:
                    ...
                    mask = s_data.astype(str).str.strip().str.lower().apply(
                        lambda x: x.replace('.0', '') not in ['0', 'hayir', 'hayır', 'false', 'pasif', 'nan', 'none']
                    ).astype(bool)
                    matched_df = matched_df[mask].copy()
                    console.print(f"  [green]✅ Temizlendi![/green] (Bu işlemle {before_len - len(matched_df)} satır daha ayıklandı)")
        ...
        with open(istasyon_file, "r", encoding="utf-8") as f:
        ...
        if os.path.exists(operasyon_file):
            with open(operasyon_file, "r", encoding="utf-8") as f:
        ...
        if os.path.exists(kapasite_file):
            try:
                with open(kapasite_file, "r", encoding="utf-8") as f:
        ...
        if is_capacity_mode:
                ...
                try:
                    ...
                    if h_idx != -1:
                        ...
                        for c in df_orig_clean.columns:
                            nc = norm_col(str(c))
                            if nc in ["kod", "malzeme kodu"]: kod_col_orig = c
                        ...
                        if stock_basis == "fiziki" and not kull_stok_col_in_df:
                            for c in df_orig_clean.columns:
                                nc = norm_col(str(c))
                                if "stok" in nc and "kullanilabilir" not in nc and "rezerve" not in nc:
                                    kull_stok_col_in_df = c
                                    break
                            ...
                            for _, row in df_orig_clean.iterrows():
                                ...
                                if k and k != 'NAN' and k != '':
                                    if pd.notna(row[kull_stok_col_in_df]):
                                        try:
                                            stok_dict_kullanilabilir[k] = float(str(row[kull_stok_col_in_df]).replace(",", "."))
                                        except:
                                            pass
                except Exception as e:
                    console.print(f"  [yellow]Uyarı:[/yellow] Orijinal dosyadan stok okuma hatası: {e}")
            ...
            if os.path.exists(stok_path):
                try:
                        ...
                        for _, row in df_stok.iterrows():
                            ...
                            if k and k != 'NAN' and k != '':
                                try:
                                    ...
                                    fiziki_val = 0.0
                                try:
                                    rezerve_val = float(str(row[rezerve_col]).replace(",", ".")) if rezerve_col and pd.notna(row[rezerve_col]) else 0.0
                                except:
                                    rezerve_val = 0.0
                    ...
                    console.print(f"  [yellow]Uyarı:[/yellow] StokListesi veritabanından stok okuma hatası: {e}")
        ...
        console.print(f"  [cyan]⏳ İstasyon verileri işleniyor ve gruplara ayrılıyor...[/cyan]")
        ...
        for c in matched_df.columns:
            if "operasyon" in norm_col(c) and ("sira" in norm_col(c) or "no" in norm_col(c)) and "adi" not in norm_col(c):
                operasyon_sira_col = c
                break

        # --- YENİ EKLENTİ: ROTASIZLAR SAYFASI İÇİN FİLTRELEME ---
        console.print(f"  [cyan]⏳ Rotasız parçalar tespit ediliyor ve ayrıştırılıyor...[/cyan]")
        source_df = raw_df if raw_df is not None else filtered_df
        op_cols = [c for c in source_df.columns if settings.col_operasyon_keyword.lower() in str(c).lower()]
        ...
        if raw_df is not None:
            console.print(f"  [cyan]⏳ Montaj izleme ilişkileri ve hiyerarşi ağacı analiz ediliyor...[/cyan]")
            ...
            # --- DEBUG ---
            op_cols = [c for c in raw_df.columns if settings.col_operasyon_keyword.lower() in str(c).lower()]
            ...
            if op_col and sira_col:
                raw_df['norm_op'] = raw_df[op_col].astype(str).str.strip().str.upper().str.replace(" ", "")
                ...
                miktar_col = None
                for c in [settings.col_rezerve_miktar, settings.col_kullanilabilir_stok, "Çarpılmış Miktar", "Carpilmis Miktar"]:
                ...
                filt_id_col = raw_kod_col if (raw_kod_col and raw_kod_col in filtered_df.columns) else sira_col
                    ...
                    for _, r in filtered_df.iterrows():
                        ...
                        if k_val and k_val != 'nan':
                            try:
                                ...
                                qty_val = 0.0
                            kod_to_remaining_qty[k_val] = kod_to_remaining_qty.get(k_val, 0.0) + qty_val
                
                # Eşleşen veritabanı üzerinden TIM operasyonuna sahip olan tüm parça kodlarının setini alalım
                tim_codes_in_db = set(
                    depo_df[depo_df['Operasyon Adı'].astype(str).str.strip().str.upper().isin(target_ops_for_raw)]['Kod']
                ...
                def _is_tim(row):
                    ...
                    for oc in [c for c in raw_df.columns if settings.col_operasyon_keyword.lower() in str(c).lower()]:
                ...
                if not montaj_parents_df.empty and not tim_rows_raw.empty:
                    ...
                    raw_qty_col = None
                    for col_name in ["Miktar", "Çarpılmış Miktar", "Carpilmis Miktar",
                                     settings.col_rezerve_miktar, "Üretilecek Miktar"]:
                    ...
                    for idx, row in tim_rows_raw.iterrows():
                        ...
                        if "KAYNAK DOSYA" in montaj_parents_df.columns and "KAYNAK DOSYA" in row:
                            ...
                            parents = montaj_parents_df[montaj_parents_df[sira_col].astype(str).isin(prefixes)]
                        if not parents.empty:
                            ...
                            p_row = parents.loc[parents[sira_col].astype(str).apply(lambda x: len(x.split('.'))).idxmax()]
                            ...
                            # Orijinal miktar
                            try:
                                orig_qty = float(str(row.get(miktar_col, row.get(settings.col_rezerve_miktar, 1))).replace(',', '.'))
                            except ValueError:
                                orig_qty = 1.0
                            ...
                            for qc in ["Miktar", "Adet", "Qty", "Quantity", miktar_col]:
                                if qc in row:
                                    ...
                                    break
                            try:
                                design_qty = float(str(row.get(design_qty_col or miktar_col, 1)).replace(',', '.'))
                            except ValueError:
                                design_qty = orig_qty
                    ...
                    for rel in matched_tim_relations:
                        ...
                        parent_kod = str(rel['p_row'].get('Kod', rel['p_row'].get(sira_col, ''))).strip()
                        ...
                        key_val = rel['parca_kodu'] if filt_id_col == raw_kod_col else rel['sira_no']
                        ...
                        for rel in rel_list:
                            ...
                            if alloc_qty > 0:
                                ...
                                montaj_izleme_dict[t_sheet].append({
                                    'Kaynak Dosya': rel['row_ref'].get('KAYNAK DOSYA', kaynak_metin_kisa),
                                    'Üst Montaj Kodu': rel['p_row'].get('Kod', rel['p_row'].get(sira_col, '')),
                                    'Üst Montaj Adı': rel['p_row'].get('Malzeme', rel['p_row'].get('Malzeme Adı', '')),
                    ...
                    if montaj_izleme_dict:
                        ...
                        for sheet_name, rows in montaj_izleme_dict.items():
                            ...
                            df_izleme = df_izleme.groupby(['Kaynak Dosya', 'Üst Montaj Kodu', 'Üst Montaj Adı', 'Alt Parça Kodu', 'Alt Parça Adı'], as_index=False).agg({'Gereken Miktar': 'sum', 'Üretilen Miktar': 'sum', 'Tamamlanma Oranı (%)': 'max'})
                            if not df_izleme.empty:
                                temp_prio = df_izleme['Kaynak Dosya'].apply(lambda x: calculate_row_priority(x, priority_mapping))
                                df_izleme = df_izleme.assign(tmp_prio=temp_prio).sort_values(by=['tmp_prio', 'Kaynak Dosya', 'Üst Montaj Kodu', 'Alt Parça Kodu']).drop(columns=['tmp_prio'])
        ...
        if "Ad" in matched_df.columns: uretim_cols.append("Ad")
        elif "Malzeme Adı" in matched_df.columns: uretim_cols.append("Malzeme Adı")
        elif "Malzeme" in matched_df.columns: uretim_cols.append("Malzeme")
        ...
        if "Çarpılmış Miktar" in matched_df.columns: uretim_cols.append("Çarpılmış Miktar")
        if "Üretilecek Miktar" in matched_df.columns: uretim_cols.append("Üretilecek Miktar")
        ...
        console.print(f"  [cyan]⏳ Üretim takip sayfası hazırlanıyor...[/cyan]")
        ...
        if filtered_kod_sutunu and settings.col_depo_kod in depo_df.columns:
            depo_kodlar = set(depo_df[settings.col_depo_kod].dropna().astype(str).str.strip().str.upper())
            filt_kodlar = ham_takip_df[filtered_kod_sutunu].dropna().astype(str).str.strip().str.upper()
            gecerli_maske = filt_kodlar.isin(depo_kodlar)
            takip_gecerli = ham_takip_df[gecerli_maske].copy()
            
            carp_col = None
            for c in ["Çarpılmış Miktar", "Carpilmis Miktar", settings.col_rezerve_miktar, settings.col_kullanilabilir_stok]:
            ...
            if carp_col: cols_to_keep.append(carp_col)
            
            takip_df = takip_gecerli[cols_to_keep].copy()
            
            rename_dict = {filtered_kod_sutunu: "Kod"}
            if carp_col: rename_dict[carp_col] = "Üretilecek Miktar"
            ...
            if "KAYNAK DOSYA" in takip_df.columns:
                takip_df["KAYNAK DOSYA"] = takip_df["KAYNAK DOSYA"].astype(str).str.strip().apply(
        ...
        def convert_numbers_to_real(df):
            def to_real_number(val):
                ...
                if val_str.lstrip('-').isdigit(): return int(val_str)
        ...
        for s_name in list(sheet_dfs.keys()):
            ...
            if s_name not in ["ÜRETİM LİSTESİ", "HAMMADDE SİPARİŞ", "Tüm Veriler", "HAMMADDE", "Üretim Takip", "Rotasızlar"] and "ZLEME" not in s_name.upper():
        ...
        console.print(f"  [cyan]⏳ Excel sayfaları diske yazılıyor (Pandas)...[/cyan]")
        with pd.ExcelWriter(out_path, engine='openpyxl') as writer:
            ...
            if "HAMMADDE SİPARİŞ" in sheet_dfs:
                sheet_dfs["HAMMADDE SİPARİŞ"].to_excel(writer, sheet_name="HAMMADDE SİPARİŞ", index=False)
            ...
            for sheet_name in sorted_sheet_names:
                ...
                if sheet_name not in ["HAMMADDE", "HAMMADDE SİPARİŞ", "Üretim Takip", "Rotasızlar"] and "ZLEME" not in sheet_name.upper():
        ...
        target_hide_cols = [
            "birimagirlik", "receteonaydurumu", "receteaktifmi", 
            "rotaadi", "anarotami", "rotaaktifmi", "rotaistikameti",
            "operasyonadi", "rotaoperasyononaydurumu", "istasyonanakayitmi", "fasonmu",
            "rezerveedilecekmiktar", "onceliksirasi"
        ]
        ...
        station_extra_hide = ["hammaddemiktar", "toplamhammaddemiktari", "isistasyonu", "operasyonsirano"]
        ...
        try:
            ...
            console.print(f"  [cyan]ℹ️ Dinamik Excel Limiti Ayarlandı: Maksimum Sayfa Satır Sayısı = {max_df_len}, Belirlenen Limit = {global_max_row}[/cyan]")
            console.print(f"  [cyan]⏳ Excel dosyası openpyxl ile yükleniyor (Stiller ve formüller uygulanacak)...[/cyan]")
            wb = openpyxl.load_workbook(out_path)
            wb.parent_stats = parent_stats
            gizlenen_adet = 0
            
            # Dosyadaki TÜM sheetler için aynı ayarları uygula
            console.print(f"  [cyan]⏳ Formüller enjekte ediliyor, hücre stilleri ve koşullu biçimlendirmeler uygulanıyor...[/cyan]")
            for ws_name in wb.sheetnames:
                ws = wb[ws_name]
                is_station_sheet = ws_name not in ["Tüm Veriler", "HAMMADDE", "HAMMADDE SİPARİŞ", "Üretim Takip", "ÜRETİM LİSTESİ", "Rotasızlar"] and "ZLEME" not in ws_name.upper()
                ...
                for col in ws.columns:
                    ...
                    hide_this_col = False
                    if ws_name == "Üretim Takip" and col_letter in ["A", "B", "C", "D", "E", "F", "G", "H"]:
                        hide_this_col = True
                    elif header_val:
                ...
                for col in ws.columns:
                    ...
                    if val == "Üretilecek Miktar": uretilecek_col_letter = col[0].column_letter
                    elif val == "Hammadde Miktar": ham_miktar_col_letter = col[0].column_letter
                    elif val == "Toplam Hammadde Miktarı": toplam_col_letter = col[0].column_letter
                    elif val == settings.col_hammadde_kod: hk_col_letter = col[0].column_letter
                    elif val == settings.col_depo_kod: dk_col_letter = col[0].column_letter
                    elif val == "Hazırlık Süresi": hazirlik_col_letter = col[0].column_letter
                    elif val == "Birim İşlem Süresi" or val == "Birim İşlem": birim_col_letter = col[0].column_letter
                    elif val == "Toplam Süre": toplam_sure_col_letter = col[0].column_letter
                    elif val == "Setup Yükü (%)": setup_yuku_col_letter = col[0].column_letter
                    elif val == "Önerilen Verimli Adet": onerilen_adet_col_letter = col[0].column_letter
                    elif val == "Güncel Setup Yükü (%)": guncel_setup_col_letter = col[0].column_letter
                ...
                if ws_name in ["HAMMADDE SİPARİŞ", "HAMMADDE SİPARİŞİ"]:
                    if hk_col_letter and toplam_col_letter and hasattr(wb, 'hammadde_kod_harfi') and hasattr(wb, 'hammadde_toplam_harfi'):
                    ...
                    if uretilecek_col_letter and dk_col_letter:
                        ...
                        if "Üretilecek Miktar" in actual_uretim_cols:
                            lookup_idx = actual_uretim_cols.index("Üretilecek Miktar") - idx_of_kod_in_uretim + 1
                        else:
                    ...
                    if uretilecek_col_letter and hazirlik_col_letter and birim_col_letter and toplam_sure_col_letter:
                    ...
                    if uretilecek_col_letter and hazirlik_col_letter and birim_col_letter:
                        if setup_yuku_col_letter:
                            for row_idx in range(2, ws.max_row + 1):
                                ws[f"{setup_yuku_col_letter}{row_idx}"] = f"={hazirlik_col_letter}{row_idx}/({uretilecek_col_letter}{row_idx}*{birim_col_letter}{row_idx})"
                                ws[f"{setup_yuku_col_letter}{row_idx}"].number_format = '0%'
                        
                        if onerilen_adet_col_letter and setup_yuku_col_letter:
                            for row_idx in range(2, ws.max_row + 1):
                                ws[f"{onerilen_adet_col_letter}{row_idx}"] = f"=IF({setup_yuku_col_letter}{row_idx}<=0.15, {uretilecek_col_letter}{row_idx}, ROUNDUP({hazirlik_col_letter}{row_idx}/({birim_col_letter}{row_idx}*0.15), 0))"
                        
                        if guncel_setup_col_letter and onerilen_adet_col_letter:
                            for row_idx in range(2, ws.max_row + 1):
                                ws[f"{guncel_setup_col_letter}{row_idx}"] = f"={hazirlik_col_letter}{row_idx}/({onerilen_adet_col_letter}{row_idx}*{birim_col_letter}{row_idx})"
                                ws[f"{guncel_setup_col_letter}{row_idx}"].number_format = '0%'

                        # --- YENİ EKLENTİ: Setup Yükü Veri Çubukları ve %15 Uyarıları ---
                        setup_databar = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=1, color="FFC000")
                        setup_alert = CellIsRule(operator='greaterThan', formula=['0.15'], font=Font(color='FFFF0000', bold=True))
                        
                        if setup_yuku_col_letter:
                            ws.conditional_formatting.add(f"{setup_yuku_col_letter}2:{setup_yuku_col_letter}{ws.max_row}", setup_databar)
                            ws.conditional_formatting.add(f"{setup_yuku_col_letter}2:{setup_yuku_col_letter}{ws.max_row}", setup_alert)
                            
                        if guncel_setup_col_letter:
                            ws.conditional_formatting.add(f"{guncel_setup_col_letter}2:{guncel_setup_col_letter}{ws.max_row}", setup_databar)
                            ws.conditional_formatting.add(f"{guncel_setup_col_letter}2:{guncel_setup_col_letter}{ws.max_row}", setup_alert)

                    # --- YENİ EKLENTİ: Üretim Takip'te üretilenleri istasyonlarda yeşile boyama ---
                    if dk_col_letter:
                        green_fill_row = PatternFill(start_color='92D050', end_color='92D050', fill_type='solid')
                        green_font_row = Font(color='006100', bold=True)
                        cf_formula = [f"COUNTIF('Üretim Takip'!$I$2:$I${global_max_row}, ${dk_col_letter}2)>0"]
                        green_rule = FormulaRule(formula=cf_formula, stopIfTrue=False, fill=green_fill_row, font=green_font_row)
                    ...
                    if toplam_sure_col_letter:
                        ...
                        ws[f"{toplam_sure_col_letter}{hour_row}"] = f"=SUM({toplam_sure_col_letter}2:{toplam_sure_col_letter}{last_row})/3600"
                        ...
                        ws[f"{toplam_sure_col_letter}{day_row}"] = f"={toplam_sure_col_letter}{hour_row}/({gunluk_saat}*{toplam_sure_col_letter}{mach_row})"
                    ...
                    for col in ws.columns:
                        ...
                        if not val: continue
                        val_str = str(val).strip()
                        if val_str == "KAYNAK DOSYA": kaynak_col_idx = col[0].column_letter
                        if val_str == "Kod": kod_col_idx = col[0].column_letter
                        if val_str == "Üretilecek Miktar": uretilecek_col_idx = col[0].column_letter
                        if val_str == "Üretilen Miktar": uretilen_col_idx = col[0].column_letter
                        if val_str == "Kalan Miktar": kalan_col_idx = col[0].column_letter
                        if val_str == "Tamamlanma (%)": tamamlanma_col_idx = col[0].column_letter
                        
                    if kod_col_idx and uretilecek_col_idx and uretilen_col_idx and kalan_col_idx and tamamlanma_col_idx:
                        ...
                        for row_idx in range(2, ws.max_row + 1):
                            ...
                            ws[f"{tamamlanma_col_idx}{row_idx}"] = f"=IF({c_uretilecek}>0, {c_uretilen}/{c_uretilecek}, 0)"
                        ...
                        for row_idx in range(2, max(ws.max_row + 500, 1000)):
                            ws[f"K{row_idx}"] = f'=IF(I{row_idx}<>"", MAX(0, SUMIF($I$2:$I${global_max_row}, I{row_idx}, $J$2:$J${global_max_row}) - SUMIF(${kod_col_idx}$2:${kod_col_idx}${global_max_row}, I{row_idx}, ${uretilecek_col_idx}$2:${uretilecek_col_idx}${global_max_row})), "")'
                        ...
                        if kaynak_col_idx:
                            ...
                            for i, kaynak in enumerate(unique_kaynaklar, start=2):
                                ...
                                ws[f"P{i}"] = f'=COUNTIFS({kaynak_col_idx}$2:{kaynak_col_idx}${global_max_row}, "*"&M{i}&"*", {kalan_col_idx}$2:{kalan_col_idx}${global_max_row}, ">0")'
                                
                                if orig_kalem > 0:
                                    ws[f"N{i}"] = orig_kalem
                                    ws[f"O{i}"] = f"=MAX(0, N{i}-P{i})"
                                    ws[f"Q{i}"] = f"=IF(N{i}>0, (O{i} + P{i} * IF({sumif_uretilecek}>0, {sumif_uretilen}/{sumif_uretilecek}, IF(P{i}>0, 0, 1))) / N{i}, 0)"
                                    ws[f"Q{i}"].number_format = '0.00%'
                                else:
                                    ws[f"N{i}"] = f'=COUNTIF({kaynak_col_idx}$2:{kaynak_col_idx}${global_max_row}, "*"&M{i}&"*")'
                                    ws[f"O{i}"] = f"=MAX(0, N{i}-P{i})"
                                    ws[f"Q{i}"] = f"=IF({sumif_uretilecek}>0, {sumif_uretilen}/{sumif_uretilecek}, 0)"
                                    ws[f"Q{i}"].number_format = '0.00%'
                        ...
                        databar_green = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=1, color="5CB85C")
                        databar_blue = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=1, color="638EC6")
                        
                        if tamamlanma_col_idx:
                            ws.conditional_formatting.add(f"{tamamlanma_col_idx}2:{tamamlanma_col_idx}{ws.max_row}", databar_green)
                        ...
                        if kaynak_col_idx and unique_kaynaklar:
                            ...
                            ws.conditional_formatting.add(q_range, CellIsRule(operator='lessThan', formula=['0.2'], fill=red_fill_cell))
                            ws.conditional_formatting.add(q_range, CellIsRule(operator='between', formula=['0.2', '0.8'], fill=yellow_fill_cell))
                            ws.conditional_formatting.add(q_range, CellIsRule(operator='greaterThan', formula=['0.8'], fill=green_fill_cell))
                        
                        # --- YENİ EKLENTİ: HATALI KOD GİRİŞİNDE KIRMIZI UYARI (Koşullu Biçimlendirme) ---
                        if kod_col_idx:
                            red_fill = PatternFill(start_color='FFFF0000', end_color='FFFF0000', fill_type='solid')
                            cf_rule = FormulaRule(formula=[f'AND($I2<>"", COUNTIF(${kod_col_idx}$2:${kod_col_idx}${global_max_row}, $I2)=0)'], stopIfTrue=True, fill=red_fill)
                            ws.conditional_formatting.add(f"I2:I{max(ws.max_row + 500, 1000)}", cf_rule)
                    ...
                    if toplam_sure_col_letter:
                        ...
                        ws[f"{toplam_sure_col_letter}{sum_saniye_row}"] = f"=SUM({toplam_sure_col_letter}2:{toplam_sure_col_letter}{last_data_row})"
                        ...
                        ws[f"{toplam_sure_col_letter}{sum_saat_row}"] = f"={toplam_sure_col_letter}{sum_saniye_row}/3600"
                        ...
                        ws[f"{toplam_sure_col_letter}{sum_gun_row}"] = f"={toplam_sure_col_letter}{sum_saat_row}/{gunluk_kapasite}"
            ...
            for sheet_name in wb.sheetnames:
                ...
                for col in ws_format.columns:
                    ...
                    if val in ["kod", "alt parça kodu", "alt parca kodu", "hammadde kod", "hammadde kodu"] and code_col_letter is None:
                ...
                if code_col_letter:
                    for row_idx in range(2, ws_format.max_row + 1):
                        cell_val = str(ws_format[f"{code_col_letter}{row_idx}"].value).strip().upper()
                ...
                if "ZLEME" in sheet_name.upper():
                    ...
                    alt_fill = PatternFill(start_color='D2E3FC', end_color='D2E3FC', fill_type='solid')
                    ...
                    for i, parent in enumerate(unique_parents):
                        ...
                        stats = parent_stats.get(key, {'total': set(), 'missing': set(), 'total_qty': 0.0})
                        ...
                        if total_count == 0:
                            ws_format[f"N{write_row}"] = f"=COUNTIFS(A:A, K{write_row}, B:B, L{write_row})"
                            ws_format[f"O{write_row}"] = f"=COUNTIFS(A:A, K{write_row}, B:B, L{write_row}, H:H, 1)"
                            ws_format[f"Q{write_row}"] = f"=SUMIFS(F:F, A:A, K{write_row}, B:B, L{write_row})"
                        else:
                            ws_format[f"N{write_row}"] = total_count
                            if in_stock_count > 0:
                                ws_format[f"O{write_row}"] = f"={in_stock_count}+COUNTIFS(A:A, K{write_row}, B:B, L{write_row}, H:H, 1)"
                            else:
                                ws_format[f"O{write_row}"] = f"=COUNTIFS(A:A, K{write_row}, B:B, L{write_row}, H:H, 1)"
                        ...
                        ws_format[f"P{write_row}"] = f"=IF(N{write_row}>0, O{write_row}/N{write_row}, 0)"
                        ...
                        for rel in parent_relations:
                            if rel.get('allocated_qty', 0.0) <= 0:
                                ...
                                for qc in ["Miktar", "Adet", "Qty", "Quantity"]:
                                    if qc in rel['p_row']:
                                        try:
                                            p_qty = float(str(rel['p_row'].get(qc, 1)).replace(',', '.'))
                                            break
                                        except ValueError:
                                            pass
                        ...
                        if in_stock_tim_limit is not None:
                            ...
                            ws_format[f"R{write_row}"] = f"=IF(P{write_row}=1, _xlfn.MINIFS(I:I, A:A, K{write_row}, B:B, L{write_row}), 0)"
            ...
            for sheet_name in wb.sheetnames:
                if "ZLEME" in sheet_name.upper():
                    ...
                    for col in list(ws_izleme.columns)[:8]:
                        ...
                        if val == "Alt Parça Kodu": alt_kod_col = col[0].column_letter
                        if val == "Gereken Miktar": gereken_col = col[0].column_letter
                        if val == "Üretilen Miktar": uretilen_col = col[0].column_letter
                        if val == "Tamamlanma Oranı (%)": oran_col = col[0].column_letter
                    
                    if alt_kod_col and uretilen_col and hasattr(wb, 'ut_kod_col') and hasattr(wb, 'ut_uretilen_col'):
                        ...
                        for row_idx in range(2, main_max_row + 1):
                            ...
                            ws_izleme[f"{oran_col}{row_idx}"] = f"=IF({req_qty}>0, {uretilen_col}{row_idx}/{req_qty}, 0)"
                            ...
                            if matching_rel:
                                ...
                                for qc in ["Miktar", "Adet", "Qty", "Quantity", miktar_col]:
                                    if qc in matching_rel['p_row']:
                                        try:
                                            p_qty = float(str(matching_rel['p_row'].get(qc, 1)).replace(',', '.'))
                                            break
                                        except ValueError:
                                            pass
                                c_design_qty = matching_rel.get('design_qty', matching_rel['orig_qty'])
                                birim_ihtiyac = c_design_qty / p_qty if p_qty > 0 else 0.0
                                
                            if birim_ihtiyac > 0:
                                val_eldeki = stok_dict_kullanilabilir.get(child_str, 0.0)
                                val_ihtiyac = total_required_qty.get(child_str, 0.0)
                                sumif_str = f"SUMIF('Üretim Takip'!I:I, {alt_kodu}, 'Üretim Takip'!J:J)"
                        ...
                        green_rule = FormulaRule(formula=[cf_formula], stopIfTrue=False, fill=green_fill, font=green_font)
                        ...
                        databar_izleme = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=1, color="5CB85C")
                        ws_izleme.conditional_formatting.add(f"{oran_col}2:{oran_col}{main_max_row}", databar_izleme)
                        ...
                        databar_izleme_side = DataBarRule(start_type='num', start_value=0, end_type='num', end_value=1, color="5CB85C")
                        ws_izleme.conditional_formatting.add(f"P2:P{side_max_row}", databar_izleme_side)
                        
                        # Yan tablonun tamamlanan gruplarını (%100 tamamlanma oranı) yeşile boya
                        green_fill_side = PatternFill(start_color='92D050', end_color='92D050', fill_type='solid')
                        green_font_side = Font(color='006100', bold=True)
                        cf_formula_side = 'AND($K2<>"", $P2=1)'
                        green_rule_side = FormulaRule(formula=[cf_formula_side], stopIfTrue=False, fill=green_fill_side, font=green_font_side)
            ...
            for sheet_name in wb.sheetnames:
                ...
                for col in ws.columns:
                    ...
                    for cell in col:
                        ...
                        if val is not None:
                            ...
                            if val_str.startswith('='):
                                if any(x in val_str.upper() for x in ['COUNTIFS', 'SUMIF', 'SUMIFS', 'IF', '/']):
                                    val_len = 6
                                else:
                                    val_len = 10
                            else:
                    ...
                    padding = 5 if is_izleme and col_letter in ['K', 'L', 'M', 'N', 'O', 'P', 'Q'] else 4
                    adjusted_width = max(max_len + padding, 12)
                    ws.column_dimensions[col_letter].width = adjusted_width

            # Final Kaydetme
            console.print(f"  [cyan]⏳ Excel dosyası kaydediliyor (Bu işlem biraz zaman alabilir)...[/cyan]")
            wb.save(out_path)
            wb.close()
            if gizlenen_adet > 0:
                console.print(f"  [cyan]👁️  İstediğiniz {gizlenen_adet} özel sütun Excel'deki TÜM SAYFALARDA gizlendi.[/cyan]")
        except Exception as e:
            console.print(f"  [yellow]Uyarı:[/yellow] Sütun biçimlendirme sırasında hata oluştu: {e}")
                ...
                if excel_files:
                    ...
                    for fpath in excel_files:
                        ...
                        for h in [0, 1, 2, 3]:
                            try:
                                ...
                                cols_norm = [str(c).strip().lower().replace("ı","i").replace("ş","s").replace("ç","c").replace("ğ","g") for c in candidate.columns]
                            ...
                            try:
                                ...
                                if ut_df is not None and not ut_df.empty:
                                    ...
                                    if "KAYNAK DOSYA" in ut_df.columns:
                                        file_mask = ut_df["KAYNAK DOSYA"].astype(str).str.strip().apply(
                                            lambda x: x == fname_no_ext or x == fname or x.startswith(fname_no_ext)
                                        )
                                        file_ut_df = ut_df[file_mask]
                                    else:
                                        file_ut_df = ut_df
                                    
                                    if not file_ut_df.empty and "Kod" in file_ut_df.columns and "Üretilecek Miktar" in file_ut_df.columns:
                                        for _, row in file_ut_df.iterrows():
                                            k = str(row["Kod"]).strip().upper()
                                            try:
                                                val = float(str(row["Üretilecek Miktar"]).replace(",", "."))
                                            except Exception:
                                                val = 0.0
                                            if k and k != "NAN" and k != "":
                                ...
                                tree_dict = build_bom_tree(df_tree, filename=displayName, initial_reqs=initial_reqs)
                                
                                # Makine adını çekip displayName'e ekle
                                machine_name = ""
                                if tree_dict:
                                    ad_val = str(tree_dict.get("ad", "")).strip()
                                    if ad_val and ad_val.upper() not in ["ROOT", "ANA MAKİNE", displayName.upper(), "ANA MAKINE"]:
                                        machine_name = ad_val
                                    else:
                                ...
                                console.print(f"  [yellow]Uyarı:[/yellow] {fname} için ağaç oluşturulamadı: {e}")
                    ...
                    if all_trees:
                        ...
                        if os.path.exists(template_path):
                            ...
                            html_content = html_content.replace("__TREE_DATA_PLACEHOLDER__", json.dumps(all_trees[0]["tree"], ensure_ascii=False))
                            ...
                            console.print(f"[green]✅ Reçete Ağacı oluşturuldu:[/green] {out_tree_path}")
            except Exception as e:
                console.print(f"[yellow]Uyarı: Otomatik ağaç oluşturma sırasında hata oluştu:[/yellow] {e}")
        ...
        panel = Panel(table, title=f"[bold green]Eşleştirme Raporu[/bold green]", border_style="green", expand=False)
        console.print(panel)
        console.print(f"[green]✅ Çıktı oluşturuldu:[/green] {out_path}\n")
...
@app.command()
def agac(path: str = typer.Argument(..., help="Makine Reçetesi Excel dosyası veya klasörü")) -> None:
    ...
    for fpath in excel_files:
        ...
        if df is None or df.empty:
            console.print(f"  [yellow]Uyari:[/yellow] {fname} okunamadi veya recete sutunu bulunamadi, atlaniyor.")
            continue
        try:
            prio, _, base_name = get_file_priority_and_name(fpath, prio_mapping)
            displayName = base_name
            ...
            all_trees.append({"name": displayName, "tree": tree_dict})
            console.print(f"  [green]OK[/green] Agac olusturuldu.")
        except Exception as e:
    ...
    console.print(f"[bold yellow]>>> Lutfen su dosyaya cift tiklayarak acin:[/bold yellow] {out_path}\n")
