...
def apply_exclusions(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], int]:
    ...
    if not os.path.exists(json_path):
        console.print(f"[yellow]Uyarı:[/yellow] {json_path} bulunamadı, hariç tutma işlemi atlanıyor.")
        return df, [], 0
    ...
    if kod_col and sira_col:
        ...
        ust_sira_prefixleri = [str(s) + "." for s in hedef_sira_nolar if str(s).strip() and str(s).strip() != "nan"]
        ...
        silinecek_maske = df_filtered.apply(lambda r: is_deleted(r['__temp_kod__'], r['__temp_sira__']), axis=1)
...
def filter_id_based(df: pd.DataFrame, target_group: set[str]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    op_cols = [str(c) for c in df.columns if settings.col_operasyon_keyword.lower() in str(c).lower()]
    ...
    df = df.copy()
    df['__target_num__'] = df[hedef_sutun].astype(str).str.replace(',', '.').astype(float).fillna(0.0)
    ...
    if brut_col and not df_orijinal_grup.empty and brut_col in df_orijinal_grup.columns:
        brut_series = pd.to_numeric(df_orijinal_grup[brut_col].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
        ...
        # Fallback
        carp_series = pd.to_numeric(df_orijinal_grup[carp_col].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
...
def filter_stock_based(df: pd.DataFrame, target_group: set[str]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
