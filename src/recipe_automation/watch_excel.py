...
def parse_excel_takip_safe(path):
    ...
    try:
        ...
        for col in df.columns:
            ...
            col_norm = col_norm.replace("ı", "i").replace("ş", "s").replace("ç", "c").replace("ğ", "g").replace("ü", "u").replace("ö", "o")
            if col_norm == "kod":
                kod_col = col
        ...
        if not kod_col or not req_col or not prod_col:
            ...
            print("[UYARI] Üretim Takip sütunları otomatik çözümlenemedi. Varsayılanlar aranıyor...")
            return None
...
def update_html_with_excel_data(html_path_val, excel_data):
    ...
    try:
        with open(html_path_val, "r", encoding="utf-8") as f:
            html_content = f.read()
        ...
        new_html_content = html_content[:match.start(2)] + new_json_str + html_content[match.end(2):]
                ...
                if current_mtime > last_mtime:
                    ...
                    if excel_data:
                        ...
                        if success:
                            ...
                            print(f"[BİLGİ] {time.strftime('%H:%M:%S')} - Excel verileri okundu ve HTML güncellendi.")
        except Exception as e:
...
def start_server(port=8000):
    ...
    with socketserver.TCPServer(("", port), WatcherHTTPHandler) as httpd:
        print(f"[SUNUCU] Canlı izleme adresi: http://localhost:{port}/{os.path.basename(html_path)}")
